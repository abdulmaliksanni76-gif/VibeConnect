...
//   // 3. Optional: Socket logic can remain here
//   if (socket) {
//     socket.emit("join_chat", conversationId);
//     // ... rest of your socket listeners ...
//   }

//   // Return either nothing or a cleanup function
//   return () => {
//     // Cleanup if necessary
//   };
// }, [conversationId, socket]); // Dependencies
...
//   // useEffect(() => {
//   //   // FIX: Changed to a standard function declaration to avoid the async useEffect warning
//   //   const fetchData = () => {
...
// //       if (infoRes.ok) setChatData(await infoRes.json());
// //     } catch (err) {
...
// //   // 3. Optional: Add socket cleanup here
// // }, [conversationId, socket]); // Dependencies
...
//   // FIX: Added safety checks (optional chaining and existence checks) to resolve "Cannot read properties of null"
//   const groupedMessages = [...messages].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).reduce((groups, message) => {
//     const date = new Date(message.createdAt).toDateString();
//     if (!groups[date]) groups[date] = [];
//     groups[date].push(message);
...
//       <div className="input-area">
//         <button className="plus-btn" onClick={(e) => { e.stopPropagation(); setShowFileMenu(!showFileMenu); }}><Plus size={24} /></button>
//         <button className={`mic-btn ${isRecording ? 'recording' : ''}`} onClick={isRecording ? stopRecording : startRecording} type="button">
//           {isRecording ? <X size={20} /> : <Mic size={20} />}
//         </button>
